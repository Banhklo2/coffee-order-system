...
인기 메뉴 조회 과정에서
최근 7일 데이터 조회 시 Full Table Scan이 발생할 수 있는 구조였습니다.
...
특히 인기 메뉴는
조회는 많고, 변경은 적으며, 동일한 결과가 반복 조회되는 특징이 있었습니다.

→ Redis 캐싱을 도입하여 반복 조회를 최적화했습니다.
...
- 이벤트 기반 구조 검증
- 불필요한 repository 호출 방지 검증 (never())

또한 주문 생성 시 동시성 테스트를 통해  
...
이번 프로젝트를 통해 설계를 먼저 꼼꼼하게 진행한 후 구현을 시작하는 과정의 중요성을 느낄 수 있었습니다.  
또한 단순 기능 구현을 넘어 트랜잭션, 동시성, 성능 개선까지 단계적으로 고민하며 설계하는 경험이 인상 깊었습니다.

특히 직접 적용해보는 과정에서 문제를 단계적으로 해결하는 것이 중요하다는 것을 느꼈고,  
